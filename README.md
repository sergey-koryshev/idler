...
> Currently, the feature is supported only in area to add new note. To increase accuracy, keep unused categorized hidden. Any change in categories affects auto-categorization logic, so if anything has changed in category description or visibility, please retrain the model in Settings (the `Refresh` icon in auto-categorization feature section).
